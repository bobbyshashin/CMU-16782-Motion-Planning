...
void AStarPlanner::addCellToOpenList(const std::pair<int, int> coordinate, int time, const Cell& cell) {
    f_to_coordinate.insert({cell.F, std::make_tuple<coordinate.first, coordinate.second, time});
    open_list.insert({std::make_tuple<coordinate.first, coordinate.second, time>, cell});
}

void AStarPlanner::addCellToClosedList(const std::pair<int, int> coordinate, int time, const Cell& cell) {
    closed_list.insert({std::make_tuple<coordinate.first, coordinate.second, time>, cell});
};
...
AStarPlanner::Cell AStarPlanner::constructCell(std::pair<int, int> coordinate, int time, int parent_x, int parent_y) {
    int x = coordinate.first;
    int y = coordinate.second;
    AStarPlanner::Cell new_cell(map[x][y].cost + map[parent_x][parent_y].cost, getHeuristic(x, y), time, parent_x, parent_y);
    return new_cell;
}

bool AStarPlanner::isInOpenList(std::tuple<int, int, int> coordinate) {
    return open_list.find(coordinate) != open_list.end();
}

bool AStarPlanner::isInClosedList(std::tuple<int, int, int> coordinate) {
    return closed_list.find(coordinate) != closed_list.end();
}

bool AStarPlanner::hasBeenExplored(std::tuple<int, int, int> coordinate) {
    return isInOpenList(coordinate) || isInClosedList(coordinate);

}

std::pair<int, int> AStarPlanner::getNextAction(const std::pair<int, int>& goal, int time) {
    int x = goal.first;
    int y = goal.second;
    int counter = 0;
    ...
    while (true) {
        auto cell_itr = closed_list.find(std::tuple(x, y, time));
