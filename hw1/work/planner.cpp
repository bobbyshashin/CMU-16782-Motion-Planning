        ...
        )
{
    ...
    if (!global_dijkstra.isInitialized()) {
        ...
        for (int i=0; i<target_steps; ++i) {
            ...
            int y = target_traj[i + target_steps] - 1;
        ...
        for (const auto& point : global_dijkstra.target_trajectory) {
            ...
            if (target_arrival_time > curr_time) {
                ...
                if (wait_time > 1) {
                    global_dijkstra.evaluated_paths.insert({path.cost + wait_time*global_dijkstra.map[coordinate.first][coordinate.second].cost, path});
                }
            }
